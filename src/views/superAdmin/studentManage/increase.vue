        ...
        phone: [
          { required: true, message: "请输入手机号码", trigger: "blur" },
          {
            validator: function(rule, value, callback) {
              if (
                /(^1[3|4|5|6|7|8|9]\d{9}$)|(^09\d{8}$)/.test(value) == false
              ) {
          ...
          for (let i = 0; i < zhicollege.length; i++) {
            ...
            for (let j = 0; j < zhimajor.length; j++) {
              if (zhicollege[i].collegeId == zhimajor[j].collegeId) {
                ...
                for (let k = 0; k < zhimatbClass.length; k++) {
                  if (zhimajor[j].majorId == zhimatbClass[k].majorId) {
                    Bosput[i].children[j].children.push({
                      value: zhimatbClass[k].tbClassId,
                      label: zhimatbClass[k].tbClassName,
                      majorId: zhimatbClass[k].majorId
                    });
                  }
                }
              }
        ...
        if (valid) {
          ...
          if (
            ...
            addUser({
              ...
              tbClassId: this.ruleForm.valueopt[2] //班级
            })
              .then(res => {
                console.log(res);
                if (res.code == 200) {
    ...
    cloBol: function() {
      if (this.cloBol) {
        if (this.ctype == "" || this.ctype == undefined || this.ctype == null) {
          ...
          this.ruleForm.valueopt[2] = this.ctype.tbClassId;
